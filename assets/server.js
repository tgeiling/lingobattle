...
app.get('/battle/requests/:username', async (req, res) => {
  try {
      ...
      const user = await User.findOne({ username }).select("battleRequests");

      if (!user) {
          return res.status(404).json({ message: "User not found." });
      }

      // Return the list of battle requests
      res.status(200).json({ battleRequests: user.battleRequests || [] });
  } catch (error) {
...
app.get('/getBattleRequests/:username', async (req, res) => {
  const { username } = req.params;
  console.log(`🔹 Fetching battle requests for ${username}`);
    ...
    if (activeBattles[matchId]) {
        ...
        if (player) {
            ...
            if (battle.players.every((p) => p.correctAnswers !== undefined)) {
                ...
                // ✅ **Update existing match in MongoDB**
                try {
                    await MatchResult.findOneAndUpdate(
                        { matchId: matchId },
                        {
                            $set: {
                                "players.$[p1].correctAnswers": player1.correctAnswers,
                                "players.$[p1].progress": player1.progress,
                                "players.$[p2].correctAnswers": player2.correctAnswers,
                                "players.$[p2].progress": player2.progress,
                            }
                        },
                        {
                            arrayFilters: [
                                { "p1.username": player1.username },
                                { "p2.username": player2.username }
                            ],
                            new: true
                        }
                    );

                    console.log(`[DATABASE UPDATE] Match ${matchId} results updated.`);
                } catch (error) {
                    console.error(`[DATABASE ERROR] Failed to update match ${matchId} results: ${error}`);
                }
